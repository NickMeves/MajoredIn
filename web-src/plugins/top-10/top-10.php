	...
	if ( $results ) {
		...
		foreach ( $results as $result ) {
			...
			if ( ! $p_in_c ) {
				...
				$mi_thumbnail = tptn_get_the_post_thumbnail( array(
				        'postid' => $result->ID,
				...
				$mi_has_thumb = empty($mi_thumbnail) ? false : true;
				...
				if ( $mi_has_thumb && 'inline' == $post_thumb_op || 'after' == $post_thumb_op || 'thumbs_only' == $post_thumb_op ) {
					$output .= '<a href="' . get_permalink( $postid ) . '" rel="' . $rel_attribute . '" ' . $target_attribute . 'class="tptn_link">'; // Add beginning of link
					$output .= $mi_thumbnail;
					$output .= '</a>'; // Close the link
				}
				...
				if ( 'inline' == $post_thumb_op || 'text_only' == $post_thumb_op ) {
				    if ($mi_has_thumb) {
				        ...
				        $output .= '<span class="tptn_no_thumb">';
				    }
					$output .= '<a href="' . get_permalink( $postid ) . '" rel="' . $rel_attribute . '" ' . $target_attribute . 'class="tptn_link">'; // Add beginning of link
					$output .= '<span class="tptn_title">' . $post_title . '</span>'; // Add title when required by settings
					$output .= '</a>'; // Close the link
				}
				//TODO: END MAJOREDIN EDIT HERE.  MERGE ON ALL UPDATES
